...
def stumpClassify(dataMatrix,dimen,threshVal,threshIneq):#just classify the data
    retArray = ones((shape(dataMatrix)[0],1))  # 默认全设1
    if threshIneq == 'lt': # 小于
        retArray[dataMatrix[:,dimen] <= threshVal] = -1.0   # 对指定特征中，小于等于阈值的分类设为 -1, 即预测为 -1
    else:
        retArray[dataMatrix[:,dimen] > threshVal] = -1.0  # 大于
    return retArray
#找到最佳决策树.
# 实际就是循环每个特征，在每个特征内按照最大值、最小值做间隔，循环每个间隔。系统计算每个特征、每个间隔时，分别用小于等于和大于做分类判别，哪一次准确率最高就用哪个.
    ...
    for i in range(n):#遍历数据集所有特征
        ...
        for j in range(-1, int(numSteps) + 1):  #遍历不同步长时的情况
            for inequal in ['lt', 'gt']:  #大于/小于阈值 切换遍历
                ...
                weightedError = D.T * errArr  # 前面预测正确的值为0，因此预测正确的最终权重较低.
...
d,c = loadDataSet("horseColicTraining2.txt")

classifierArr = adaBoostTrainDS(d,c, numIt=200)

d,c = loadDataSet("horseColicTest2.txt")
print(d)
print(c)
print("------------------------------------")
print(adaClassify(d,classifierArr))
